(ns remote.main
  "Kotodama WASM entrypoint for the Remote clean-room actor (L4) — Clojure port.
  ...
  facts (`remote.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "remote")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"remote.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "remote-compat" :tier tier :entities entities} 200])
...
(defn start! [] :remote-compat/ready)
